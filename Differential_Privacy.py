...
def local_train(global_model, train_loader, epochs=1, lr=0.001, device="cpu", noise_scale=0.01):
    ...
    # --- Differential Privacy Noise Added Here ---
    ...
    return noisy_weights, len(train_loader.dataset)

# -------------------------
# Step 4.4: FedAvg aggregation
# -------------------------
def fedavg(local_weights, local_sizes):
    """Weighted average of model weights from clients"""
    global_weights = copy.deepcopy(local_weights[0])
    total_size = sum(local_sizes)

    for key in global_weights.keys():
        global_weights[key] = sum(
            (local_weights[i][key] * local_sizes[i] for i in range(len(local_weights)))
        ) / total_size
...
# -------------------------
# Step 4.6: Federated training rounds
# -------------------------
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
global_model = SimpleCNN().to(device)

num_rounds = 3  # try 3 rounds first
for rnd in range(1, num_rounds+1):
    print(f"\n--- Federated Round {rnd} ---")
    
    # Each client trains locally with DP
    w_a, n_a = local_train(global_model, loader_a, epochs=1, device=device, noise_scale=0.01)
    w_b, n_b = local_train(global_model, loader_b, epochs=1, device=device, noise_scale=0.01)
    w_c, n_c = local_train(global_model, loader_c, epochs=1, device=device, noise_scale=0.01)
